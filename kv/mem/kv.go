...
func (k *service) BitSet(key string, idx int, val bool, ttl time.Duration) error {
	...
	k.data[key] = obj{
		TTLKey: k.updateTTL(key, v.TTLKey, ttl),
		Kind:   v.Kind,
		Value:  d,
	}
	return nil
...
func (k *service) updateTTL(key string, oldTTL string, ttl time.Duration) string {
	if _, ok := k.Collector[oldTTL]; ok {
		delete(k.Collector[oldTTL], key)
	}
	return k.setTTL(key, ttl)
}

func (k *service) setTTL(key string, ttl time.Duration) string {
