...
build-and-unit-test:
  ...
  steps:
    ...
    # docs/tutorial/18-testing-strategy.md for why the split matters:
...
is the CI enforcement of the warning chapter 14 raised when introducing `servo generate`: if
...
Chapter 18 calls this out as a real diagnostic to watch for — this job is what makes it visible in
the first place.
...
the exact same problem chapter 20's `depends_on: condition: service_healthy` solves for local
...
This builds the same image chapter 20 builds and runs locally, from the same repo-root context, for
...
this tutorial: `go build`/`go vet`/`go test` in chapter 18, `servo check` in chapter 14, the exact
`docker build` invocation in chapter 20. The workflow's job is to run those same, already-proven
...
- **testcontainers-go instead of a `services:` block** (see also chapter 18's alternatives) — would
...
[Chapter 20: Running and deployment](20-running-and-deployment.md) — the `Dockerfile` and
`docker-compose.yml` this chapter's `docker-build` job and chapter 18's `make up` both depend on,
