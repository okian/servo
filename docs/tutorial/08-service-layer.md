...
`*observability.Logger` is built in [chapter 16](16-observability.md) and threaded through here
...
[Why the logger is a node](16-observability.md#why-the-logger-is-a-node-and-not-a-call-at-the-top-of-main)
...
that situation in [chapter 14](14-wiring-with-servo.md#capabilities-side-by-side). Here, testing
...
15](16-observability.md) is where that logger stops being the unconfigured default and starts
...
- **Don't** test `OrderService` by spinning up real Postgres/Redis/NATS. That's what
  [chapter 18](18-testing-strategy.md)'s integration and API tiers are for — a unit test that needs
