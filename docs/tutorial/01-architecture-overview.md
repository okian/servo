...
| Config | Typed structs, loaders generated by `//servo:config` | Fails fast on missing config with generate-time tag validation; no reflection library and no stringly-typed `os.Getenv` scattered around |
