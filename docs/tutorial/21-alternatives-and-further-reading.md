# 21. Alternatives and further reading
...
topic, meant to be read selectively (or searched, the way [chapter 20](20-troubleshooting.md) is)
rather than start to finish. Chapters 17, 18, and 19 already have their own `## Alternatives`
...
every handler harder to test without pulling gin into the test too. This is the one alternative on
this page you don't have to imagine: [chapter 11](11-gin-transport.md) builds this service's API in
Gin, so the trade is measurable rather than argued.
...
actually diverge. [Chapter 12](12-grpc-transport.md) is that both, serving gRPC and REST from a
single port against the same service layer.

## Structured logging libraries

`observability/logging.go` uses `log/slog`, the stdlib structured logger added in Go 1.21 —
covered fully in chapter 15. Before `slog` existed, [Uber's
...
Prometheus (chapter 15) and OpenTelemetry are both vendor-neutral by design, which is precisely why
...
`resilience.RateLimiter` (chapter 16) is one shared token bucket for the entire process — every
...
`session.Session` (chapter 14) is one instance per user, generated by servo. Three other shapes
...
`docker-compose.yml` (chapter 19) is explicitly a local-development convenience — one host, one
...
- **`/healthz` and `/readyz` become `livenessProbe` and `readinessProbe`** on the `Deployment`
  spec, polling the exact same admin-port endpoints chapter 10 and 15 already built — no new code,
...
- **`JWT_SECRET` becomes a `Secret`**, mounted as an environment variable via `secretKeyRef` rather
  than written into a manifest or a compose file in plain text (chapter 19's own do's and don'ts
...
chapters 10 and 15 already built for exactly this reason.
...
Chapter 17 already covers table-driven-vs-separate-function tests, testcontainers-go, and contract
...
- [Chapter 14](14-scoped-instances.md): Redis, a hand-written registry, or recomputing per request.
- [Chapter 17](17-testing-strategy.md#alternatives): testcontainers-go, table-driven tests, contract testing.
- [Chapter 18](18-cicd.md#alternatives): combined vs. split CI jobs, `actions/cache`, other CI systems, Dependabot.
- [Chapter 19](19-running-and-deployment.md#alternatives): image registries, plain `docker run` vs. Compose, debug sidecars.
...
- [OpenTelemetry documentation](https://opentelemetry.io/docs/) — this tutorial's tracing setup
  (chapter 15) uses a small slice of a much larger spec, including metrics and logs signals this
...
patterns the last twenty chapters already established. Go build the next thing on top of it.
