# 22. Alternatives and further reading
...
topic, meant to be read selectively (or searched, the way [chapter 21](21-troubleshooting.md) is)
...
covered fully in chapter 16. Before `slog` existed, [Uber's
...
Prometheus (chapter 16) and OpenTelemetry are both vendor-neutral by design, which is precisely why
...
`resilience.RateLimiter` (chapter 17) is one shared token bucket for the entire process — every
...
`session.Session` (chapter 15) is one instance per user, generated by servo. Three other shapes
...
`docker-compose.yml` (chapter 20) is explicitly a local-development convenience — one host, one
...
- **`JWT_SECRET` becomes a `Secret`**, mounted as an environment variable via `secretKeyRef` rather
  than written into a manifest or a compose file in plain text (chapter 20's own do's and don'ts
...
Chapter 18 already covers table-driven-vs-separate-function tests, testcontainers-go, and contract
...
- [Chapter 15](15-scoped-instances.md): Redis, a hand-written registry, or recomputing per request.
- [Chapter 18](18-testing-strategy.md#alternatives): testcontainers-go, table-driven tests, contract testing.
- [Chapter 19](19-cicd.md#alternatives): combined vs. split CI jobs, `actions/cache`, other CI systems, Dependabot.
- [Chapter 20](20-running-and-deployment.md#alternatives): image registries, plain `docker run` vs. Compose, debug sidecars.
...
- [OpenTelemetry documentation](https://opentelemetry.io/docs/) — this tutorial's tracing setup
  (chapter 16) uses a small slice of a much larger spec, including metrics and logs signals this
