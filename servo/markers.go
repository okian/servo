...
// Marker is the opaque return type of Root, Bind, Override, Scoped, HTTP,
// and Extract. It carries no data; `servo generate` reads calls to these
// functions as syntax inside a Build(...) argument list and never executes
// them.
type Marker struct{}

// Build declares an injector: its roots, explicit bindings, scopes, HTTP
// servers, and extractors. Calls to it are read as syntax by
// `servo generate`, in a file carrying the servoinject build tag that is
// excluded from the compiled binary. If Build ever runs, the tag was
// missing or generation was skipped — panic rather than silently returning
// a nil app.
func Build(...Marker) {
