...
// Ready reports whether this server is accepting connections yet — the
// same "send me traffic" signal as the other two transports, and for both
// protocols at once, since they share the listener.
func (s *Server) Ready(context.Context) error {
	if !s.ready.Load() {
		return errors.New("grpcapi: not accepting connections yet")
	}
	return nil
}
