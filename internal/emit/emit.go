...
type emitter struct {
	...
	http *httpEmit
	// configFileFn and configFileVar name the generated config-file loader
	// and the local its result lands in — set only when the spec declares
	// a servo.ConfigFile and the graph uses a config.
	configFileFn  string
...
func Emit(resolved *resolve.Resolved, spec *load.Spec, testMode bool) ([]byte, error) {
	...
	// The config-file loader and its local, claimed under the same rule:
	// only a graph that reads a config file pays for the names.
	e.planConfigs()
	...
	// Config values load right after them, and are locals exactly the way
	// node variables are, so they are named and shadow-checked the same
	// way.
	all := append(append([]*resolve.Node{}, resolved.Supplied...), resolved.Configs...)
	all = append(all, resolved.Order...)
	...
	body.WriteString(configFileDecl)
	...
	body.WriteString(httpHandlerFunc)
...
// buildConstraint is the //go:build expression for the emitted file,
// computed by load from the spec file's own constraint and the tags the
// graph was resolved under. A Spec built by hand — every test in this
// package does that — carries none, and falls back to the plain mirror
// image of the spec's tag, which is what a tagless generation emits.
func (e *emitter) buildConstraint() string {
	if e.spec.GeneratedConstraint != "" {
		return e.spec.GeneratedConstraint
	}
	return "!" + load.BuildTag
}
...
func (e *emitter) header() string {
	...
	for _, n := range e.resolved.Configs {
		fmt.Fprintf(&b, "//   [L0] %s\n//         servo:config (prefix %s)  %s\n", n.Key.String(), n.Config.Prefix, e.posString(n.Config.Pos))
	}
