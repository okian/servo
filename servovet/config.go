		...
		for _, group := range file.Comments {
			for _, c := range group.List {
				...
				if name != graph.ConfigDirective {
					// The route feature owns the HTTP verbs — validate its
					// grammar here (attachment and cross-route conflicts stay
					// with the generator). A name that is neither config nor a
					// route verb is a genuine typo, and since only servovet
					// sees both directive families at once, it gives the one
					// message that names them both.
					if route.IsDirectiveMethod(name) {
						if problem, ok := route.CheckDirectiveComment(c.Text); !ok {
							pass.Reportf(c.Pos(), "%s", problem)
						}
						continue
					}
					pass.Reportf(c.Pos(), "servo: unrecognized directive //servo:%s — the directives are //servo:%s on a type and the route verbs (get, post, put, patch, delete, head, options) on a function", name, graph.ConfigDirective)
					continue
				}
